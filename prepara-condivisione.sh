...
# Percorsi base calcolati dinamicamente: niente nome utente hard-coded.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"          # cartella assisted-labs
DESKTOP_DIR="${HOME}/Desktop"

OUTPUT_DIR="${DESKTOP_DIR}/assisted-labs-condivisione"
BUNDLE_ARCHIVE="${DESKTOP_DIR}/assisted-labs-collega.tar.gz"

echo "==> [1/4] Preparazione cartella di condivisione in $OUTPUT_DIR..."
mkdir -p "$OUTPUT_DIR"

echo "==> [2/4] Esportazione immagine Docker ($IMAGE_NAME)..."
docker save "$IMAGE_NAME" | gzip > "$OUTPUT_DIR/alpine-latest-assisted-labs.tar.gz"

echo "==> [3/4] Esportazione volume /workspace ($VOLUME_NAME)..."
docker run --rm \
...
echo "==> [4/4] Creazione script e guida di avvio automatico per il collega..."
...
# Copia anche i sorgenti se presenti
if [ -d "$PROJECT_DIR" ]; then
    tar -czf "$OUTPUT_DIR/assisted-labs-sorgenti.tar.gz" -C "$PROJECT_DIR/.." assisted-labs
fi

echo "==> [5/5] Creazione archivio finale compresso in $BUNDLE_ARCHIVE..."
tar -czf "$BUNDLE_ARCHIVE" -C "$DESKTOP_DIR" assisted-labs-condivisione

echo "==> COMPLETATO! Pacchetto pronto da inviare: $BUNDLE_ARCHIVE"
